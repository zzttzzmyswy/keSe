...
uint32_t touch_ad(void) {
	...
	if (value_touch < 46000 && value_t[0] > 0xD00 && value_t[3] > 0x1000) {
		port[0] = (double)(value_t[0] - 0xE00) / (0xEA00 - 0xE00) * 480;
		port[0] = port[0] <= 0 ? -1 : port[0] >= 480 ? 480 : port[0];
		port[1] = (double)(value_t[3] - 0x1C00) / (0xD800 - 0x1C00) * 272;
		port[1] = port[1] <= 0 ? -1 : port[1] >= 272 ? 272 : port[1];
		return 1;
	}
	else {
		port[0] = -1;
