...
void Touch_Button_Init(void) {
	...
	button[15].para = LINE_WITH_CIRCLE;
	...
	button[17].end_y = PALETTE_END_Y;
	button[17].para = RUBBER;
	button[17].touch_flag = 0;
	button[17].draw_btn = Draw_Rubber_Button ;
...
void Touch_Button_Down(uint16_t x, uint16_t y) {
	...
	for (i = 0; i < BUTTON_NUM; i++) {
		/* 触摸到了按钮 */
		if (x <= button[i].end_x && y <= button[i].end_y && y >= button[i].start_y
			&& x >= button[i].start_x) {
...
void Touch_Button_Up(uint16_t x, uint16_t y) {
	...
	if (x > PALETTE_START_X) {
		...
		/* 描绘1像素宽度的轨迹线 */
		case RECT:
			if (x - 10 < PALETTE_START_X) //矩形
				break;
			if (Rect_x != x && Rect_y != y) {
				if (!(x > PALETTE_END_X - 10 || y > PALETTE_END_Y - 10 || x < 10 || y < 10))
...
void Draw_Trail(int16_t pre_x, int16_t pre_y, int16_t x, int16_t y,
	...
	if (x > PALETTE_START_X && pre_x > PALETTE_START_X) {
		...
		case LINE_16_PIXCEL:
			...
			break;
		case RECT:
			if (x - 10 < PALETTE_START_X || pre_x - 10 < PALETTE_START_X) //矩形
				break;
			if (Rect_x == -1) {
				if (!(x > PALETTE_END_X - 10 || y > PALETTE_END_Y - 10 || x < 10 || y < 10)) {
					Rect_x = x;
		...
		case RUBBER:
			if (x - 20 < PALETTE_START_X ||                 //画板左边界
				x + 20 > PALETTE_END_X || x - 20 < 0 || //液晶左右边界
				y + 20 > PALETTE_END_Y || y - 20 < 0) //液晶上下边界
				break;
			//        if(x>PALETTE_START_X+20)
			{
				LCD_SetColors(CL_WHITE, CL_WHITE);
				LCD_DrawFullRect(x - 30 / 2,
					y - 30 / 2,
					30,
					30);
			}
...
/**
* @brief  Draw_Rubber_Button 按钮的描绘函数
* @param  btn Touch_Button 类型的按键参数
* @retval 无
*/
static void Draw_Rubber_Button(void *btn) {
...
/**
* @brief  Draw_Rect_Button 按钮的描绘函数
* @param  btn Touch_Button 类型的按键参数
* @retval 无
*/
static void Draw_Rect_Button(void *btn) {
...
static void LCD_DrawUniLineCircle(uint16_t x1, uint16_t y1, uint16_t x2,
	...
	for (curpixel = 0; curpixel <= numpixels; curpixel++) {
		//判断边界
		if (x + thick > PALETTE_END_X || x - thick < 0 || //液晶左右边界
			y + thick > PALETTE_END_Y || y - thick < 0) //液晶上下边界
			continue;
