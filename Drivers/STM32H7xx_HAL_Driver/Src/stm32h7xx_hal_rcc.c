...
__weak HAL_StatusTypeDef HAL_RCC_OscConfig(RCC_OscInitTypeDef  *RCC_OscInitStruct) {
	...
	if (((RCC_OscInitStruct->OscillatorType) & RCC_OSCILLATORTYPE_HSE) == RCC_OSCILLATORTYPE_HSE) {
		...
		if ((temp_sysclksrc == RCC_CFGR_SWS_HSE) || ((temp_sysclksrc == RCC_CFGR_SWS_PLL1)
	...
	if (((RCC_OscInitStruct->OscillatorType) & RCC_OSCILLATORTYPE_HSI) == RCC_OSCILLATORTYPE_HSI) {
		...
		const uint32_t temp_sysclksrc = __HAL_RCC_GET_SYSCLK_SOURCE();
		const uint32_t temp_pllckselr = RCC->PLLCKSELR;
		if ((temp_sysclksrc == RCC_CFGR_SWS_HSI) || ((temp_sysclksrc == RCC_CFGR_SWS_PLL1)
				&& ((temp_pllckselr & RCC_PLLCKSELR_PLLSRC) == RCC_PLLCKSELR_PLLSRC_HSI))) {
	...
	if (((RCC_OscInitStruct->OscillatorType) & RCC_OSCILLATORTYPE_CSI) == RCC_OSCILLATORTYPE_CSI) {
		...
		const uint32_t temp_sysclksrc = __HAL_RCC_GET_SYSCLK_SOURCE();
		const uint32_t temp_pllckselr = RCC->PLLCKSELR;
		if ((temp_sysclksrc == RCC_CFGR_SWS_CSI) || ((temp_sysclksrc == RCC_CFGR_SWS_PLL1)
				&& ((temp_pllckselr & RCC_PLLCKSELR_PLLSRC) == RCC_PLLCKSELR_PLLSRC_CSI))) {
		...
		if (pllm != 0U) {
			...
			case RCC_PLLSOURCE_HSI:  /* HSI used as PLL clock source */
				if (__HAL_RCC_GET_FLAG(RCC_FLAG_HSIDIV) != 0U) {
					...
					pllvco = ((float_t)hsivalue / (float_t)pllm) * ((float_t)(uint32_t)(RCC->PLL1DIVR & RCC_PLL1DIVR_N1) + (fracn1 /
...
void HAL_RCC_GetOscConfig(RCC_OscInitTypeDef  *RCC_OscInitStruct) {
	...
	if (HAL_GetREVID() <= REV_ID_Y)
		RCC_OscInitStruct->CSICalibrationValue = (uint32_t)(READ_BIT(RCC->HSICFGR,
					HAL_RCC_REV_Y_CSITRIM_Msk) >> HAL_RCC_REV_Y_CSITRIM_Pos);
	else
		RCC_OscInitStruct->CSICalibrationValue = (uint32_t)(READ_BIT(RCC->CSICFGR,
...
#if defined(RCC_VER_X)
	if (HAL_GetREVID() <= REV_ID_Y)
		RCC_OscInitStruct->HSICalibrationValue = (uint32_t)(READ_BIT(RCC->HSICFGR,
					HAL_RCC_REV_Y_HSITRIM_Msk) >> HAL_RCC_REV_Y_HSITRIM_Pos);
	else
		RCC_OscInitStruct->HSICalibrationValue = (uint32_t)(READ_BIT(RCC->HSICFGR,
